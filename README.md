...
使用 Python 版本 >= 3.10.0（已放宽版本限制, 支持 3.10 / 3.11 / 3.12 / 3.13）
...
其余可选配置见 `config.yml` 注释：`temperature`（默认 0，推理模型可填 `null` 不下发）、`max_tokens`、
`timeout`、`max_retries`（网络错误与限速时指数退避重试）、`few_shot`（附带同题型作答示例）、
`cache`（缓存同一道题的作答结果，重复题目不再重复计费）。
