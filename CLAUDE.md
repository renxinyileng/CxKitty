...
- 现有实现: `json.py` (本地 JSON)、`sqlite.py` (本地库)、`restapi.py` (通用 REST + Enncy/网课小工具/题库海/冷月/Muke/柠檬等第三方)、`openai.py` (OpenAI 兼容大模型在线答题, 需 `base_url` / `api_key` / `model` / `system_prompt` / `prompt` 模板)。
