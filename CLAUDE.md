...
- `openai.py` 顶部的 `PROVIDERS` 是服务商预设表 (base_url / 默认模型 / 是否需要 key), 文件末尾的 `DeepSeekSearcher` 等子类只覆盖一个 `PROVIDER` 字段。**新增服务商 = 往 `PROVIDERS` 加一条 → 加一个子类 → 注册进 `SEARCHERS` → config.yml 与 README 补说明**; 服务商换了默认模型名时只需改 `PROVIDERS`。
